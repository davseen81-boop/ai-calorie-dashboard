...
const TOOLS: JarvisTool[] = [
...

  {
    spec: {
      name: "set_day_type",
      description:
        "Mark today a rest day, a normal day, or a training day ('active'). " +
        "Rest lowers the target, active raises it, and normal is the plain daily " +
        "goal — the weekly average — for someone who would rather track one " +
        "steady figure. Separate from logged exercise; the macro grams scale " +
        "with whichever applies. Without an explicit choice the day follows the " +
        "user's weekly training plan, so only call this when they say today " +
        "differs from usual.",
      parameters: {
        type: "object",
        properties: {
